...
c - Update 2016 07 28 to:
c     For d3 plots, to drop all contours
c     For d3 plots to only use "coverage" part *only* if "ij" is not "1"
c     Same for "09" and "ete" grids

c     subroutine coplotwcv(ele,fname,bw,be,bs,bn,jm,b1,b2,maxplots,
c    *olddtm,newdtm,region,elecap,ij,cptlo,cpthi,cptin,suffixused,
c    *igridsec,fn,cvfname)
...
c ----------------------------------------------------
     ...
     *         fname(1:3).eq.'vmr'.or.fname(1:3).eq.'vsr'))then
        write(6,2)trim(fname)
        stop
      endif
    2 format('FATAL in coplotwcv.  Bad character in spots 1-3: ',a)

      if(.not.(fname(4:5).eq.'cd' .or. fname(4:5).eq.'dd'))then
        write(6,3)trim(fname)
        stop
      endif
    3 format('FATAL in coplotwcv.  Bad character in spots 4-5: ',a)

      if(fname(6:8).ne.ele)then
        write(6,4)trim(fname),ele
        stop
      endif
    4 format('FATAL in coplotwcv.  Bad match of fname / ele: ',a,1x,a)
    ...
    5 format('FATAL in coplotwcv.  Bad units in name: ',a)
    ...
    6 format('FATAL in coplotwcv.  Bad igridsec: ',i0)
   10 format('(',i0,' sec)')
   11 format(i0,'.')
...
c ------------------------------------------------------------------
c - 2016 07 28:  Scrapped the whole thing about scaling cptin.
c - Just bring in the interval for an 8 color spread, period.
      cptin = cptin6
...
c - to be deleted later.
     ...
     *trim(region),trim(fn(ij)),'method noise'


c - Commented out for 2016 02 29:
c 903 format(
c    *'grdimage ',a,'.',a,'.grd',
c    *' -Ei -Itempi.grd',
...
c - Commented out for 2016 03 01:
c 903 format(
c    *'grdimage ',a,'.',a,'.grd',
c    *' -Ei ',
c    *' -R',f0.2,'/',f0.2,'/',f0.2,'/',f0.2,
...
c - 2016 07 28...skip contours if this is a d3, 09 or "ete" plot
...
c - 2016 07 28 - Skip the coverage if this is a "d3" plot, 09 plot or "ete" plot *and*
c - we are at the overall map level of 1 ("entire")
      if(suffixused(ll-1:ll).eq.'d3' .and. ij.eq.1)goto 808
      if(suffixused(ll-1:ll).eq.'09' .and. ij.eq.1)goto 808
      if(fname(3:5).eq.'ete' .and. ij.eq.1)goto 808

c - GMT call to overlay the coverage
      write(99,909)trim(cvfname)

  909 format(
     *'psxy ',a,' -R -J -Sc0.02i -Gblack -O -K >> plot.ps')
...
c 991 format(
c    *'# -----------------------------------------------------',/,
c    *'# color plots with coverage in ',a,1x,a,1x,a,1x,a,/,
c    *'# -----------------------------------------------------',/,
c    *'echo ...color plots with coverage in ',a,1x,a,1x,a,1x,a)
